...
int main(int argc, char **argv)
{
    ...
    // 根据 poutAVCodec 的 pix_fmt 获取 poutAVCodecCtx 的 pix_fmt
    ...
    int get_picture = 0;
    int read_frames = 0;
    int bit_buffer_size = 1024 * 1024; // 256kB
    uint8_t *bit_buffer = NULL;

    struct AVFrame av_decode_frame;
    struct AVPacket av_decode_pkt;
    int32_t total_read_frames = 0;
    int32_t film_pts = 0;
    int64_t timestamp = av_rescale_q(600000000, AV_TIME_BASE_Q, pinAVFmtCtx->streams[video_stream_index]->time_base);
    res = av_seek_frame(pinAVFmtCtx, video_stream_index, timestamp, AVSEEK_FLAG_BACKWARD);
    ...
    while (1) {
        ...
        total_read_frames = total_read_frames + 1;
        ...
        if (pinAVCodecCtx->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (res > 0 && 
                get_picture > 0 && 
                avpkt.stream_index == video_stream_index) {
                film_pts = avpkt.pts;

                if (film_pts >= timestamp) {
                    fprintf(stdout, "select frame pts is %d.\n", film_pts);
                    break;
                }
            }
        }
    ...
    fprintf(stdout, "total read frames is %d.\n", total_read_frames);
    ...
    if (res == -1) {
        fprintf(stdout, "encode picture frame error.\n");
        return -1;
    }
